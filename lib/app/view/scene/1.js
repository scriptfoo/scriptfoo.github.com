define([
	...
	return SceneView.extend({
		...
		onSceneAssetsReady: function () {
			...
			this.addSceneQuests();
			this.currentQuest = this.sceneQuest.get('name');
			this.sceneQuest.get('entity').startActions(this.sceneQuest.get('actions'));
		},
		addSceneQuests: function () {
			...
			var actions = new SceneActionCollection([
				...
				new CastStateActionModel({
					castState: false
				}),
				new MoveActionModel({
					entity: this.sceneEntities.get('Friar'), 
					params: {x: 620, y: 132} 
				}),
				new WaitActionModel({
					params: 800
				...
				new SpeakActionModel({
					entity: this.Player,
					params: ['I am. I\'m here to learn more about this world. Where\'s a good place to start?']
				}),
				new CastStateActionModel({
					castState: true 
				}),
				new SpeakActionModel({
					...
					params: [
						...
						'Strings are cool. Such as "wizard hat", "wand" and "spell book".',
						'Do note the use of double quotes around the string. This is important.',
						'Try it for youself, enter "How do you do, fine Earth?" in the spell caster below:'
					]	
				}),
				new BindSpellActionModel({
					entity: this.sceneEntities.get('Friar'),
					binding: true
				}),
				new BindSpellActionModel({
				...
				new SpeakActionModel({
					entity: this.sceneEntities.get('Friar'),
					params: ['He\'s a quick one!'],
				})
			]);

			this.sceneQuest = new QuestModel({
				id: 0,
				name: 'level01',
				actions: actions
			});
		...
		addSceneEntities: function () {
			...
			var f = new FriarEntityModel({
				'sprite': 'IdleLeft',
				x: 530, y: 318,
				id: 'Friar'
			});
			this.sceneEntities.add(f);
