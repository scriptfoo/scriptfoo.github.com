define([
	...
	return SceneView.extend({
		...
		onSceneAssetsReady: function () {
			...
			this.sceneEntities = new EntityCollection(); 
		...
		onPlayerMove: function (from) {
			if(this.Player.get('entity').hitbox.hit('Solid')) {
				this.Player.get('entity').attr({x: from.x, y:from.y});
				return;
			}
		...
		addSceneQuests: function () {
			...
			var a1 = new ActionModel({
				method: 'move', 
				params: {x: 580, y: 300},
				entity: this.sceneEntities.get('Friar')
			});

			var a2 = new ActionModel({
				method: 'move', 
				params: {x: 620, y: 132},
				entity: this.sceneEntities.get('Friar')
			});

			var a3 = new ActionModel({
				method: 'move', 
				params: {x: 420, y: 130},
				entity: this.sceneEntities.get('Friar')
			});

			var actions = new ActionCollection([a1, a2, a3]);

			this.sceneQuest = new QuestEntityModel({
				index: 0,
				name: 'level01',
				text: ['hello!'],
				actions: actions
