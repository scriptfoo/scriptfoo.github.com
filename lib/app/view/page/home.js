define([
	...
	return PageView.extend({
		...
		afterRender: function () {
			...
			this.$('#cr-stage')
				.append($('<div></div>').addClass('day-states')
					.append($('<div></div>').addClass('day-state day'))
					.append($('<div></div>').addClass('day-state dayFoggy'))
					.append($('<div></div>').addClass('day-state evening'))
					.append($('<div></div>').addClass('day-state night')));
			...
			this._setUpDialogWidget();
			
			this.scenes['Scene1'] = new Scene1View({
				el: this.$('#cr-stage'),
				editorspellscriptmodel: this.editorspellscriptmodel
			});

			this.scenes['SceneMaze1'] = new SceneMazeView1({
				el: this.$('#cr-stage'),
				EditorSpellScriptModel: this.EditorSpellScriptModel
			});

			// Load any scene you want here
			this.scenes['Scene1'].loadScene();
		},
		_setUpDialogWidget: function () {
			var dialogMessagesCollection = new DialogMessagesCollection();
			...
			this.DialogWidgetView = new DialogWidgetView({
				...
				DialogMessagesCollection: dialogMessagesCollection
			});

			// Send the DialogMessagesCollection to the SpeakerComponent so it can use it easily
			Crafty.components()['Speaker'].DialogMessagesCollection = dialogMessagesCollection;
		},
