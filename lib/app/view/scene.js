	...
	return Backbone.View.extend({
		...
		_sceneDayStatesAlternator: function () {
			...
			toggleNextClass = function () {
				...
				$el.removeClass(dayStates.join(' ')).addClass(nextClass);
			};

			$el.on('transitionend webkitTransitionEnd oTransitionEnd otransitionend', toggleNextClass);
			toggleNextClass(); // Call it the first time (no animation)
			setTimeout(function () {
				$el.addClass('animated-day-transitions');
				$('.day-state', $el).addClass('animated-day-transitions');
				toggleNextClass();
			}, 2000);
		},
		/**
