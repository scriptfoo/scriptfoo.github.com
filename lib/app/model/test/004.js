define([
	...
	return TestModel.extend({
		...
		execute: function () {
			...
			this.get('SpeakerModel').say('Also, let\'s save the returned string into a new variable named \'calmReply\'.');
			...
			this.get('SpeakerModel').say({
				text: 'You know the drill, to the spell caster! save the returned string into a new variable named \'calmReply\':',
				sticky: true
			});
		},
		onCast: function (Spell, HookModel) {
			try {
				...
				this.get('SpeakerModel').say({
					text: 'That doesn\'t seem right! Set the variable \'calmReply\' to an exclaimation-free version of the variable below:',
					sticky: true,
					force: true
				});
			}
