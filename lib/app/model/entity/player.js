define([
	...
	return EntityModel.extend({
		...
		_getCurrentMovement: function() {
			if (this._movements.x.length) {
				var currentx = this._movements.x[this._movements.x.length - 1];
				var currenty = this._movements.y[this._movements.y.length - 1];
		...
		_keyup: function (e) {
			if (this._keyToDirection(e.key) !== false) {
				...
				this.get('entity').trigger('NewDirection', this._getCurrentMovement());
			}
		},
		keyMove: function(direction, key) {
			this._direction = direction;

			var newx = Math.round(Math.cos(this._direction * (Math.PI / 180)) * 1000 * this._speed.x) / 1000;
			var newy = Math.round(Math.sin(this._direction * (Math.PI / 180)) * 1000 * this._speed.y) / 1000;

			this._movements.x.push(newx);
			this._movements.y.push(newy);

			this.get('entity').trigger('NewDirection', {x: newx, y: newy});
		...
		move: function (direction, duration, callback) {
			...
			newx = Math.round(Math.cos(this._direction * (Math.PI / 180)) * 1000 * this._speed.x) / 1000;
			newy = Math.round(Math.sin(this._direction * (Math.PI / 180)) * 1000 * this._speed.y) / 1000;
		...
		_doMove: function () {
			var entity = this.get('entity');
			var currentMovement = this._getCurrentMovement();

			if (currentMovement.x !== 0) {
				entity.x += currentMovement.x;
				entity.trigger('Moved', { x: entity.x - currentMovement.x, y: entity.y });
			}

			if (currentMovement.y !== 0) {
				entity.y += currentMovement.y;
				entity.trigger('Moved', { x: entity.x, y: entity.y - currentMovement.y});
			}
		}
	});
});